...
export class Boot extends Phaser.Scene {
  ...
  private hc(col: number, row: number): { x: number; y: number } {
    return {
  ...
  // Clip ctx to hex shape centred at (cx, cy)
  private hexClip(ctx: CanvasRenderingContext2D, cx: number, cy: number, size: number): void {
    ctx.beginPath();
    for (let i = 0; i < 6; i++) {
  ...
  private generateMapTexture(tiles: number[][], key: string): void {
    ...
    sm.width = MAP_COLS * SC; sm.height = MAP_ROWS * SC;
      ...
      for (let col = 0; col < MAP_COLS; col++) {
        ...
        if (t !== 3) {
          for (const [dx, dy] of [[0,1],[0,-1],[1,0],[-1,0]]) {
            ...
            const sg = ctx.createLinearGradient(gx0, gy0, gx0 + (dx !== 0 ? ew : 0), gy0 + (dy !== 0 ? eh : 0));
            sg.addColorStop(0, 'rgba(220,200,140,0)');
            sg.addColorStop(0.4, 'rgba(200,175,110,0.55)');
            sg.addColorStop(1, 'rgba(30,20,5,0.35)');
            ctx.fillStyle = sg; ctx.fillRect(ex, ey, ew, eh);
          }
        } else {
          for (const [dx, dy] of [[0,1],[0,-1],[1,0],[-1,0]]) {
            ...
            const wg = ctx.createLinearGradient(gx0, gy0, gx0 + (dx !== 0 ? -ew : 0), gy0 + (dy !== 0 ? -eh : 0));
            wg.addColorStop(0, 'rgba(8,30,60,0)');
            wg.addColorStop(1, 'rgba(8,30,60,0.55)');
            ctx.fillStyle = wg; ctx.fillRect(ex, ey, ew, eh);
          }
        }
      }
    }

    // ── Pass 3: Organic noise ─────────────────────────────────────────────────
    for (let i = 0; i < 3500; i++) {
      const nx = this.r(i, 0, 77) * W, ny = this.r(i, 1, 77) * H;
    ...
    // ── Pass 4: Hex-centred terrain details ───────────────────────────────────
    ...
    for (let row = 0; row < MAP_ROWS; row++) {
      for (let col = 0; col < MAP_COLS; col++) {
        ...
        for (let i = 0; i < 6; i++) {
          const a = (Math.PI / 3) * i - Math.PI / 2;
          const px = cx + (HEX_SIZE - 0.5) * Math.cos(a);
          const py = cy + (HEX_SIZE - 0.5) * Math.sin(a);
          i === 0 ? ctx.moveTo(px, py) : ctx.lineTo(px, py);
        }
        ctx.closePath();
        ctx.stroke();
  ...
  private drawDetail(
    ctx: CanvasRenderingContext2D, type: number,
    cx: number, cy: number, col: number, row: number,
  ): void {
    switch (type) {
      case 0: this.dGrass(ctx, cx, cy, col, row);    break;
      case 1: this.dMountain(ctx, cx, cy, col, row); break;
      case 2: this.dForest(ctx, cx, cy, col, row);   break;
      case 3: this.dWater(ctx, cx, cy, col, row);    break;
      case 4: this.dRuins(ctx, cx, cy, col, row);    break;
      case 5: this.dRoad(ctx, cx, cy, col, row);     break;
      case 6: this.dWasteland(ctx, cx, cy, col, row); break;
      case 7: this.dTown(ctx, cx, cy);               break;
    }
  }

  // ── Terrain: Grasland ──────────────────────────────────────────────────────
  private dGrass(ctx: CanvasRenderingContext2D, cx: number, cy: number, col: number, row: number): void {
    const x = Math.round(cx - HEX_W / 2), y = Math.round(cy - HEX_SIZE), S = TILE_SIZE;
    this.hexClip(ctx, cx, cy, HEX_SIZE);
    ...
    for (let i = 0; i < 6; i++) {
      const bx = x + this.r(col + i, row, 3) * (S - 8) + 4;
      const by = y + this.r(col, row + i, 3) * (S - 8) + 4;
      for (let b = -2; b <= 2; b++) {
        const lean = (this.r(i, b, 4) - 0.5) * 5;
        const h = 5 + this.r(i + 10, b, 4) * 4;
        ctx.strokeStyle = this.r(i, b, 5) > 0.5 ? '#56a030' : '#3d8018';
        ctx.lineWidth = 1.1;
        ctx.beginPath();
  ...
  // ── Terrain: Wasser ────────────────────────────────────────────────────────
  private dWater(ctx: CanvasRenderingContext2D, cx: number, cy: number, col: number, row: number): void {
    const x = Math.round(cx - HEX_W / 2), y = Math.round(cy - HEX_SIZE), S = TILE_SIZE;
    this.hexClip(ctx, cx, cy, HEX_SIZE);

    const depthG = ctx.createRadialGradient(cx, cy, 0, cx, cy, HEX_SIZE * 0.9);
    depthG.addColorStop(0, 'rgba(8,28,55,0.35)'); depthG.addColorStop(1, 'rgba(0,0,0,0)');
    ...
    for (let wi = 0; wi < 4; wi++) {
      ...
      if (wy >= y + S) continue;
      ctx.strokeStyle = `rgba(120,190,240,${0.30 - wi * 0.04})`; ctx.lineWidth = 1.3;
      ctx.beginPath(); ctx.moveTo(x, wy);
      for (let px = 0; px <= S; px += 3) {
        ctx.lineTo(x + px, wy + Math.sin((px / S) * Math.PI * 2.8 + this.r(col, wi, 9) * Math.PI) * 2.2);
      }
      ctx.stroke();
      for (let px = 5; px < S; px += 9 + Math.round(this.r(px, wi, 10) * 6)) {
        if (this.r(px + col, wi + row, 11) > 0.55) {
          ...
          ctx.beginPath();
          ctx.ellipse(x + px, wy + Math.sin((px / S) * Math.PI * 2.8 + this.r(col, wi, 9) * Math.PI) * 2.2,
            2.2, 1, 0, 0, Math.PI * 2); ctx.fill();
        }
      }
    }
    for (let si = 0; si < 3; si++) {
      ...
      ctx.beginPath();
      ctx.ellipse(x + this.r(col + si, row, 13) * S, y + this.r(col, row + si, 13) * S,
        3.5, 1.3, -0.3, 0, Math.PI * 2); ctx.fill();
    }
  }

  // ── Terrain: Wald ─────────────────────────────────────────────────────────
  private dForest(ctx: CanvasRenderingContext2D, cx: number, cy: number, col: number, row: number): void {
    const x = Math.round(cx - HEX_W / 2), y = Math.round(cy - HEX_SIZE), S = TILE_SIZE;

    ctx.save(); this.hexClip(ctx, cx, cy, HEX_SIZE);
    ctx.fillStyle = 'rgba(0,10,0,0.35)'; ctx.fillRect(x, y, S, S);
    for (let i = 0; i < 5; i++) {
      ctx.fillStyle = `rgba(30,60,10,${0.4 + this.r(i, col, 16) * 0.3})`;
      ctx.beginPath();
      ctx.ellipse(x + this.r(col + i, row, 15) * (S - 4) + 2,
        y + this.r(col, row + i, 15) * (S - 4) + 2,
        3 + this.r(i, row, 17) * 3, 1.5, this.r(i, col, 18) * Math.PI, 0, Math.PI * 2); ctx.fill();
    }
    ...
    for (let ti = 0; ti < treeCount; ti++) {
      ...
      const ty = y + this.r(col, row + ti * 3, 20) * (S * 0.5) + S * 0.35;
      ...
      layers.forEach(([ox, oy, r, c]) => {
        ctx.fillStyle = c; ctx.beginPath(); ctx.arc(tx + ox, ty + oy, r, 0, Math.PI * 2); ctx.fill();
      });
      ctx.fillStyle = 'rgba(100,190,50,0.22)';
      ctx.beginPath();
      ctx.ellipse(tx - crownR * 0.25, ty - treeH - crownR * 0.1,
        crownR * 0.4, crownR * 0.28, -0.5, 0, Math.PI * 2); ctx.fill();
    }
  }

  // ── Terrain: Gebirge ──────────────────────────────────────────────────────
  private dMountain(ctx: CanvasRenderingContext2D, cx: number, cy: number, col: number, row: number): void {
    const x = Math.round(cx - HEX_W / 2), y = Math.round(cy - HEX_SIZE), S = TILE_SIZE;
    ...
    for (let pi = 0; pi < peakCount; pi++) {
      ...
      const py = y + S * 0.8;
      const ph = 28 + this.r(col + pi * 3, row, 28) * 22;
      const pw = 16 + this.r(col, row + pi * 3, 28) * 12;

      const mG = ctx.createLinearGradient(px - pw, py, px + pw, py - ph);
      mG.addColorStop(0, '#8a7860'); mG.addColorStop(0.4, '#7a6850'); mG.addColorStop(1, '#5a4a38');
      ctx.fillStyle = mG;
      ctx.beginPath();
      ctx.moveTo(px - pw, py); ctx.lineTo(px - pw * 0.15, py - ph);
      ctx.lineTo(px + pw * 0.12, py - ph + 4); ctx.lineTo(px + pw, py);
      ctx.closePath(); ctx.fill();

      ctx.fillStyle = 'rgba(15,8,0,0.40)';
      ctx.beginPath(); ctx.moveTo(px + pw * 0.1, py - ph + 5);
      ctx.lineTo(px + pw, py); ctx.lineTo(px + pw * 0.3, py); ctx.closePath(); ctx.fill();
      ...
      for (let li = 0; li < 3; li++) {
        ...
        ctx.lineTo(px + pw * (0.4 - li * 0.08), ly + 4 + li * 2); ctx.stroke();
      }

      const snowH = ph * 0.35, snowPy = py - ph;
      ctx.fillStyle = '#eef2ff';
      ctx.beginPath(); ctx.moveTo(px - pw * 0.15, snowPy);
      ctx.lineTo(px - pw * 0.28, snowPy + snowH);
      ctx.lineTo(px + pw * 0.25, snowPy + snowH - 3);
      ctx.lineTo(px + pw * 0.12, snowPy + 4); ctx.closePath(); ctx.fill();
      ctx.fillStyle = 'rgba(255,255,255,0.65)';
      ctx.beginPath(); ctx.ellipse(px - pw * 0.05, snowPy + snowH * 0.25,
        pw * 0.1, snowH * 0.12, -0.3, 0, Math.PI * 2); ctx.fill();
    }
  }

  // ── Terrain: Ruinen ───────────────────────────────────────────────────────
  private dRuins(ctx: CanvasRenderingContext2D, cx: number, cy: number, col: number, row: number): void {
    const x = Math.round(cx - HEX_W / 2), y = Math.round(cy - HEX_SIZE), S = TILE_SIZE;
    this.hexClip(ctx, cx, cy, HEX_SIZE);
    ...
    stones.forEach(([sx,sy,sw,sh], si) => {
      ...
      if (this.r(si, row, 31) > 0.4) {
        ...
        ctx.lineTo(x + sx + sw * 0.65, y + sy + sh); ctx.stroke();
      }
    });
    ...
    ctx.fillStyle = colG; ctx.fillRect(colX, y + S * 0.04, 9, S * 0.5);
    ctx.fillStyle = '#6a6258'; ctx.fillRect(colX - 2, y + S * 0.04, 13, 5);

    ctx.save(); ctx.translate(x + S * 0.42, y + S * 0.73); ctx.rotate(-0.48);
    ctx.fillStyle = '#6a6258'; ctx.fillRect(0, 0, S * 0.42, 7); ctx.restore();
  ...
  // ── Terrain: Straße ───────────────────────────────────────────────────────
  private dRoad(ctx: CanvasRenderingContext2D, cx: number, cy: number, col: number, row: number): void {
    const x = Math.round(cx - HEX_W / 2), y = Math.round(cy - HEX_SIZE), S = TILE_SIZE;
    this.hexClip(ctx, cx, cy, HEX_SIZE);

    const rG = ctx.createLinearGradient(x + S * 0.17, y, x + S * 0.83, y);
    rG.addColorStop(0,    'rgba(140,110,65,0)');
    ...
    rG.addColorStop(1,    'rgba(140,110,65,0)');
    ...
    ctx.fillStyle = 'rgba(190,160,100,0.18)'; ctx.fillRect(x + S * 0.38, y, S * 0.24, S);
    ctx.fillStyle = 'rgba(50,100,20,0.35)';
    ctx.fillRect(x, y, S * 0.16, S); ctx.fillRect(x + S * 0.84, y, S * 0.16, S);
    ...
    for (let i = 0; i < 6; i++) {
      ...
      ctx.beginPath(); ctx.ellipse(px, py,
        this.r(i,col+row,37)*2+1, this.r(i+1,col,37)*1.2+0.5,
        this.r(i,row,38)*Math.PI, 0, Math.PI * 2); ctx.fill();
    }
  }

  // ── Terrain: Ödland ───────────────────────────────────────────────────────
  private dWasteland(ctx: CanvasRenderingContext2D, cx: number, cy: number, col: number, row: number): void {
    const x = Math.round(cx - HEX_W / 2), y = Math.round(cy - HEX_SIZE), S = TILE_SIZE;
    this.hexClip(ctx, cx, cy, HEX_SIZE);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      ctx.beginPath();
      ctx.ellipse(x + this.r(col+i,row,40)*(S-6), y + this.r(col,row+i,40)*(S-6),
        this.r(i,row,42)*8+4, this.r(i+1,col,42)*4+2, this.r(i,col+row,42)*Math.PI, 0, Math.PI*2); ctx.fill();
    }
    ...
    cracks.forEach(pts => {
      ctx.beginPath(); pts.forEach(([pcx,pcy],i) => i===0?ctx.moveTo(x+pcx,y+pcy):ctx.lineTo(x+pcx,y+pcy)); ctx.stroke();
    });
    ...
    ctx.beginPath(); ctx.moveTo(x+S*0.65,y+S*0.9); ctx.lineTo(x+S*0.65+3,y+S*0.9-14);
    ctx.lineTo(x+S*0.65-4,y+S*0.9-22); ctx.moveTo(x+S*0.65+3,y+S*0.9-14); ctx.lineTo(x+S*0.65+8,y+S*0.9-20); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(x+S*0.2,y+S*0.75); ctx.lineTo(x+S*0.2+2,y+S*0.75-10); ctx.lineTo(x+S*0.2-3,y+S*0.75-15); ctx.stroke();
  }

  // ── Terrain: Stadt ────────────────────────────────────────────────────────
  private dTown(ctx: CanvasRenderingContext2D, cx: number, cy: number): void {
    const x = Math.round(cx - HEX_W / 2), y = Math.round(cy - HEX_SIZE), S = TILE_SIZE;
    this.hexClip(ctx, cx, cy, HEX_SIZE);
    ...
    ctx.fillStyle = wallG; ctx.fillRect(x+2, y+S*0.32, S-4, S*0.62);
  ...
  private generateHeroTexture(): void {
    ...
    c.strokeStyle = '#7a5a00'; c.lineWidth = 2; c.beginPath(); c.arc(20,20,15,0,Math.PI*2); c.stroke();
    c.strokeStyle = 'rgba(255,255,255,0.55)'; c.lineWidth = 1; c.beginPath(); c.arc(20,20,9,0,Math.PI*2); c.stroke();
  ...
  private generateEnemyMarkerTexture(): void {
    ...
    c.fillStyle = g; c.beginPath(); c.moveTo(20,2); c.lineTo(38,37); c.lineTo(2,37); c.closePath(); c.fill();
    c.strokeStyle = '#ff9999'; c.lineWidth = 1.5;
    c.beginPath(); c.moveTo(20,2); c.lineTo(38,37); c.lineTo(2,37); c.closePath(); c.stroke();
    c.fillStyle = 'rgba(255,255,255,0.88)'; c.beginPath(); c.arc(20,22,6,0,Math.PI*2); c.fill();
    c.fillStyle = '#880000';
  ...
  private generateResourceTextures(): void {
    ...
    [[18,20,9],[12,20,8],[24,18,7],[10,16,6],[22,24,5]].forEach(([gcx,gcy,rv]) => {
      const g = gc.createRadialGradient(gcx-2,gcy-2,0,gcx,gcy,rv);
      g.addColorStop(0,'#fff5c0'); g.addColorStop(0.5,'#ffd700'); g.addColorStop(1,'#a07800');
      gc.fillStyle=g; gc.beginPath(); gc.arc(gcx,gcy,rv,0,Math.PI*2); gc.fill();
      gc.strokeStyle='#806000'; gc.lineWidth=0.7; gc.beginPath(); gc.arc(gcx,gcy,rv,0,Math.PI*2); gc.stroke();
    });
    gt.refresh();
